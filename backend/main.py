...
app = FastAPI(
    ...
    description="""
This API lets you recommend related symptoms using Apriori Association Rules.  
Visit `/docs` for Swagger UI, or `/recommend?symptom=fever` to test directly.
""",
    version="1.0.0"
)
...
# Utility Functions
# -----------------------------
def preprocess_symptoms(Symptoms: pd.DataFrame) -> pd.DataFrame:
...
def generate_association_rules(symptoms_encoded: pd.DataFrame, min_support=0.2, min_confidence=0.8):
    frequent_itemsets = apriori(symptoms_encoded, min_support=min_support, use_colnames=True)
    rules = association_rules(frequent_itemsets, metric="confidence", min_threshold=min_confidence)
    return rules


def find_associated_symptoms(input_symptom: str, Symptoms: pd.DataFrame):
...
def root():
    return {
        "message": "Welcome to the Symptoms Recommender API",
        "docs_url": "/docs",
        "example": "/recommend?symptom=fever"
    }
...
def recommend(symptom: str = Query(..., description="Symptom string to search for")):
    """
    Recommend associated symptoms given an input symptom.
    """
